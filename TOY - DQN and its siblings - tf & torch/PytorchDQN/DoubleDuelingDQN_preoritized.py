...
def select_epilson_greedy_action(model, obs, eps_threshold):
        ...
        va = model(Variable(obs)).data
        if sample > eps_threshold:
            va = va.max(1)
            return va[0].cpu(), va[1].cpu()
        else:
            a = torch.IntTensor([[random.randrange(model.num_action)]])
            return va[0][a.numpy()].cpu(), a
...
num_episodes = 10000
experiment_dir = '../experiments/torch/PrioritizedDuelingDDQN'
...
# The replay memory
replay_memory = ReplayBuffer(replay_memory_size)
...
for i_episode in range(num_episodes):
    ...
    for t in itertools.count():
        ...
        old, action = select_epilson_greedy_action(q_estimator, state, epsilon)
        ...
        mini_batch, is_weights, idxs = replay_memory.getBatch(batch_size)
        state_batch, action_batch, reward_batch, next_state_batch, done_batch = map(np.array, zip(*mini_batch))
        ...
        # calculate qt_error 
        Q_state_action = q_estimator(state_batch).gather(1, action_batch.unsqueeze(1))
        _, next_state_actions = q_estimator(next_state_batch).max(1, keepdim=True)
        V_next_state = target_estimator(next_state_batch).gather(1, next_state_actions)
        target_Q_state_action = reward_batch.unsqueeze(1) + gamma * V_next_state * done_batch.unsqueeze(1)
